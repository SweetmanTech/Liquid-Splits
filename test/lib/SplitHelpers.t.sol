...
contract SplitHelpersTest is Test {
    ...
    function testCan_handleSomeDupeHolders(bytes32 seed, uint8 numHolders, uint8 _numUniqHolders) public {
        vm.assume(numHolders > 0);
        ...
        for (uint32 i = 0; i < addressesWithDupes.length; i++) {
            addressesWithDupes[i] = address( uint160( randAddresses[i] ) % numUniqHolders );
        }
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            vm.mockCall(
                        MOCK_NFT, abi.encodeWithSelector(IERC721.ownerOf.selector, _tokenIds[i]), abi.encode(addressesWithDupes[i])
            );
        }
        (address[] memory recipients, uint32[] memory percentAllocations) = sh.getRecipientsAndAllocations();
        ...
        uint32[] memory expectedPercentAllocations = new uint32[](expectedRecipients.length);
        uint32 sum = 0;
        for (uint256 i = 0; i < expectedPercentAllocations.length; i++) {
            for (uint256 j = 0; j < addressesWithDupes.length; j++) {
